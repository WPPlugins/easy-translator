...
Description: A plugin to translate other plugins (Yes, any other plugin) Access it by clicking <a href="tools.php?page=easy-translator/easy-translator.php">Tools &rarr; Easy Translator</a>.
Version: 1.01
  ...
  class ezTran {
    ...
    function ezTran()
    {
      ...
      if ($_POST['ezt-mailPot']) {
        ...
        $_SESSION['ezt-author'] = $author ;
        $_SESSION['ezt-authormail'] = $authormail ;
        ...
        if (strlen($authormail) >0) $mail->AddAddress($authormail, $author) ;
        ...
        $mail->Body = $head ;
        if ($mail->Send())
          $this->status = '<div class="updated">Pot file: ' . $file . ' was sent to ' .
            $author . ' (' . $authormail . ').</div> ' ;
        else
          $this->error =  $this->errMsg($mail->ErrorInfo) ;
      }
    }
...
<li>
<?php printf(__('%s uses the excellent Javascript/DHTML tooltips by %s', 'easy-adsenser'), '<b>Easy LaTeX</b>', '<a href="http://www.walterzorn.com" target="_blank" title="Javascript, DTML Tooltips"> Walter Zorn</a>.') ;
?>
</li>
</ul>
</td>
...
<?php
    ...
    function plugin_action($links, $file) {
      if ($file == plugin_basename(dirname(__FILE__).'/easy-translator.php')){
